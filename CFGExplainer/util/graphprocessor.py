...
class YANCFG:
    ...
    def parse_yancfg_sample(self, filename, label, num_classes=None, get_adj=True):
        ...
        for i, node in enumerate(graph.nodes):
            # print(node)
            if "temp" in str(node):     # if "temp" in node:
                mask.append(0)
            else:
                mask.append(1)
    ...
    def load_yancfg_data(self, path, path_mod, malware_list, debug=False):  # debug=False
        """
        ...
        for malware_name, label in malware_list.items():
            ...
            for name in names:
                ...
                graph, feat, one_hot_label, node_mask, _ = self.parse_yancfg_sample(load_path, label, 8)  # change to 13 ## change to 8
                ...
                # print("size of node_masks: ", sys.getsizeof(node_masks))
