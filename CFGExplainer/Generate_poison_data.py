...
def Generate_ACFG_Node_Attributes(G):
    ...
    for node, data in ACFG.nodes(data=True):
        if "temp" in str(node):
            continue
        data['Start'] = int(data['x'][0][0].split('[')[0], 16)
        data['End'] = int(data['x'][-1][0].split('[')[0], 16)
        data['Next'] = [int(ACFG.nodes[nextnode[1]]['x'][0][0].split('[')[0], 16) for nextnode in list(ACFG.edges(node))]
        data['Ins'] = [(int(ins[0].split('[')[0], 16), [ins[1].split(' ')[0], ins[1][len(ins[1].split(' ')[0]) + 1:]]) for ins in data['x']]
        data['Prev'] = [int(ACFG.nodes[prevnode[0]]['x'][-1][0].split('[')[0], 16) for prevnode in list(ACFG.in_edges(node))]
        feat = [0.0] * 8
